
    <div id="lp_success" class="container text-center my-3" style="display:none;">
        <i id="info_Icon" class="fas fa-check fa-6x text-success"></i>
        <p id="info_Status" class="text-center font-weight-bold mt-3">Berhasil !</p>
        <p id="info_Desc" class="text-center">Silahkan cek email anda. Kami telah mengirim link untuk mengganti email anda. </p>
    </div>
    <div id="lp_spinner" class="container text-center my-3" style="display:none;">
        <i class="fas fa-spinner fa-6x fa-spin text-blackpearl"></i>
        <p class="text-center font-weight-bold mt-3">Sedang di proses. Mohon ditunggu...</p>
    </div>
    <div id="lp_form" class="container my-3">
        <div class="form-group">
            <label for=""><b class="text-danger">*</b>Email</label>
            <input type="text" class="form-control" name="inputemail" id="lp_email" aria-describedby="lp_helpemail" placeholder="">
            <small id="lp_helpemail" class="form-text text-muted">Masukkan email akun anda disini. Kami akan mengecek email anda untuk mengirim link verifikasi ganti password atau cobalah kontak admin website ini.</small>
        </div>
    </div>
